...
export default function AddTermPage() {
  ...
  // التحقق من حالة الفصول عند فتح النافذة
  useEffect(() => {
    if (isModalOpen) {
  ...
  // دالة إنهاء الفصل الحالي للانتقال لخطوة الإضافة
  const handleEndCurrentTerm = () => {
    ...
    // بعد الإنهاء، ننتقل مباشرة لفورم الإضافة
    setActiveTerm(null);
    setModalStep('form');
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    alert('تمت إضافة الفصل وحفظه بنجاح!');
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            ...
            {modalStep === 'warning' && activeTerm && (
              <div className="p-8 text-center space-y-6">
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 leading-relaxed">
                  ...
                  يجب إنهاء هذا الفصل وأرشفته قبل أن تتمكن من فتح فصل دراسي جديد.
                </div>
            ...
            {/* خطوة نموذج الإدخال (لا يوجد فصل نشط) */}
            {modalStep === 'form' && (
              <form onSubmit={handleSave} className="p-6 space-y-5">
                <div>
                  <label className="block text-gray-700 font-bold mb-2 text-sm">اسم الفصل الدراسي <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    placeholder="مثال: الفصل الأول 2025"
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-right"
                    value={termName}
                ...
                <div>
                  <label className="block text-gray-700 font-bold mb-2 text-sm">ملاحظات (اختياري)</label>
                  <textarea 
                    rows={3}
                    placeholder="أضف أي ملاحظات أو تفاصيل إضافية هنا..."
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-right resize-none"
                    value={notes}
                ...
                <div className="pt-2 flex gap-3">
                  <button type="submit" className="flex-1 bg-[#c4a900] hover:bg-[#b39a00] text-black font-extrabold py-3.5 rounded-xl shadow-md transition-all active:scale-95 text-sm">
                    حفظ البيانات
                  </button>
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3.5 rounded-xl transition-all text-sm border border-gray-200">
                    إلغاء
                  </button>
                </div>
              </form>
