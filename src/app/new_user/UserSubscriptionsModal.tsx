...
export default function UserSubscriptionsModal({
  ...
  const [isRefreshing, setIsRefreshing] = useState(false); // تحديث صامت
  ...
  const fetchAvailableMaterials = async () => {
    try {
      ...
      if (result.success && Array.isArray(result.data)) setAvailableMaterials(result.data);
      else if (Array.isArray(result.data)) setAvailableMaterials(result.data);
    } catch (err) {
    ...
    if (newSubscription.materialid && newSubscription.type1) {
      ...
      if (material) {
        ...
        switch (newSubscription.type1) {
          case 'مقرر': price = material.unit_price || '0'; break;
          case 'اسئلة': price = material.quizall_price || '0'; break;
          case 'صوت': price = material.voice_price || '0'; break;
        }
  ...
  // ======= جلب الاشتراكات مع فصل التحميل الأولي عن التحديث الصامت =======
  const fetchSubscriptions = useCallback(async (initial = false) => {
    try {
  ...
  const handleAddSubscription = async () => {
    try {
      ...
      const result = await response.json();
      if (result.status === 'error') throw new Error(result.message || 'حدث خطأ أثناء الإضافة');

      const savedMaterialName = newSubscription.materialName;
      const savedType = newSubscription.type1;

      // ======= تحديد المخاطبة حسب الجنس =======
      const salutation = userGender === 'أنثى' ? `العزيزة ${userName}` : `العزيز ${userName}`;

      await fetchSubscriptions(false); // تحديث صامت
  ...
  const handleDeleteSubscription = async (id: number) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success === false || result.status === 'error') throw new Error(result.error || result.message || 'حدث خطأ أثناء الحذف');
      if (subToDelete) setDeletedSubscriptions(prev => [...prev, subToDelete]);
  ...
  const handleUpdateSubscription = async (id: number, newType: SubscriptionType) => {
    try {
      ...
      const response = await fetch(API_URL, {
        ...
        headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache, no-store, must-revalidate' },
        cache: 'no-store' as RequestCache,
        body: JSON.stringify({ subscription_id: id, type1: newType })
      });
      const result = await response.json();
      if (result.success === false || result.status === 'error') throw new Error(result.error || result.message || 'حدث خطأ أثناء التحديث');
      // تحديث محلي بدون إعادة fetch
      setSubscriptions(prev => prev.map(s => s.id === id ? { ...s, type1: newType } : s));
  ...
  const resetForm = () => {
    setNewSubscription({ materialid: '', materialName: '', type1: '', price1: '', dolar: '0' });
    setMaterialSearch('');
    setIsDropdownOpen(false);
  };
  ...
  const formatPrice = (price: string) => {
    if (!price) return '-';
    try {
      return new Intl.NumberFormat('ar-SY', { style: 'currency', currency: 'SYP' }).format(parseFloat(price));
    } catch { return price; }
  };
  ...
  const formatDolar = (dolar: string) => {
    if (!dolar) return '-';
    try {
      return new Intl.NumberFormat('ar-EG', { style: 'currency', currency: 'USD' }).format(parseFloat(dolar));
    } catch { return dolar; }
  };

  const getTypeBadge = (type: string) => {
    switch (type) {
      case 'مقرر': return 'bg-blue-50 text-blue-700 border-blue-100';
      case 'اسئلة': return 'bg-teal-50 text-teal-700 border-teal-100';
      case 'صوت': return 'bg-amber-50 text-amber-700 border-amber-100';
      default: return 'bg-gray-50 text-gray-700 border-gray-100';
    }
  };

  // ======= فلترة المواد مع تطبيع النص =======
  const filteredMaterials = availableMaterials.filter(m =>
    normalizeArabic(m.material_name).includes(normalizeArabic(materialSearch)) ||
    normalizeArabic(m.material_code).includes(normalizeArabic(materialSearch))
  ).slice(0, 10);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4 z-50">
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden relative z-10 flex flex-col">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200 bg-gray-50 flex-wrap gap-4 rounded-t-xl shrink-0">
          <div>
            ...
            <p className="text-gray-600 mt-1 font-medium">{userName} - ID: {userId}</p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => { setShowAddForm(true); fetchAvailableMaterials(); }}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {showAddForm && (
            <div className="mb-6 p-6 border border-blue-200 rounded-xl bg-blue-50/50 shadow-sm">
              <h3 className="text-lg font-bold text-blue-900 mb-4">تفاصيل الاشتراك الجديد</h3>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  />
                  {isDropdownOpen && materialSearch && !newSubscription.materialid && (
                      ...
                      {isMaterialsLoading ? (
                        ...
                        filteredMaterials.map(material => (
                          <div
                            ...
                            onClick={() => {
                              setNewSubscription({ ...newSubscription, materialid: material.id.toString(), materialName: material.material_name });
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={handleAddSubscription} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-bold hover:bg-blue-700 transition shadow-sm">حفظ الاشتراك</button>
                <button onClick={() => { setShowAddForm(false); resetForm(); }} className="bg-white border border-gray-300 text-gray-700 px-5 py-2.5 rounded-lg font-bold hover:bg-gray-50 transition shadow-sm">إلغاء</button>
              </div>
            </div>
          )}
          ...
          {isLoading ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center shadow-sm">
              ...
              <button onClick={() => fetchSubscriptions(true)} className="bg-red-600 text-white px-5 py-2.5 rounded-lg font-bold hover:bg-red-700 transition shadow-sm">إعادة المحاولة</button>
            </div>
            ...
            <>
              {/* ======= نسخة الحاسوب ======= */}
              <div className={`hidden md:block overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-200 transition-opacity duration-300 ${isRefreshing ? 'opacity-60' : 'opacity-100'}`}>
                <table className="w-full text-right divide-y divide-gray-200 table-auto">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {subscriptions.map((subscription, index) => (
                      <tr key={subscription.id} className={`transition ${updatingId === subscription.id ? 'opacity-50' : 'hover:bg-gray-50'}`}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          {editingSubscription?.id === subscription.id ? (
                            <select
                              ...
                              onChange={(e) => {
                                const newType = e.target.value as SubscriptionType;
                                setEditingSubscription({ ...editingSubscription, type1: newType });
                                handleUpdateSubscription(subscription.id, newType);
                              }}
                            ...
                            <span
                              className={`cursor-pointer text-sm font-bold px-3 py-1.5 rounded-lg hover:opacity-80 transition inline-block min-w-[70px] text-center border ${getTypeBadge(subscription.type1)}`}
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-extrabold text-green-600">{formatPrice(subscription.price1)}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-extrabold text-blue-600">{formatDolar(subscription.dolar)}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-600">{formatDate(subscription.created_at)}</td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <button
                            onClick={() => handleDeleteSubscription(subscription.id)}
                            disabled={updatingId === subscription.id}
                            className="text-red-500 hover:text-white border border-red-500 hover:bg-red-500 px-3 py-1.5 rounded-lg transition shadow-sm flex items-center justify-center gap-1 disabled:opacity-50"
                          >
              ...
              <div className={`md:hidden space-y-4 transition-opacity duration-300 ${isRefreshing ? 'opacity-60' : 'opacity-100'}`}>
                {subscriptions.map((subscription, index) => (
                  <div key={`mob-${subscription.id}`} className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-4 transition ${updatingId === subscription.id ? 'opacity-50' : ''}`}>

                    {/* رأس البطاقة */}
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <span className="text-gray-400 text-xs font-bold bg-gray-100 px-2 py-0.5 rounded-full">#{index + 1}</span>
                        <h4 className="font-extrabold text-gray-900 mt-1">{subscription.material_name}</h4>
                        <span className="text-xs text-gray-400">ID: {subscription.materialid}</span>
                      </div>
                      <span className={`text-xs font-bold px-2 py-1 rounded-lg border ${getTypeBadge(subscription.type1)}`}>{subscription.type1 || '-'}</span>
                    </div>

                    {/* بيانات السعر والتاريخ */}
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      <div className="bg-green-50 rounded-xl p-2 text-center border border-green-100">
                        <span className="block text-[10px] text-gray-400 font-medium mb-0.5">ل.س</span>
                        <span className="font-extrabold text-green-700 text-xs">{formatPrice(subscription.price1)}</span>
                      </div>
                      <div className="bg-blue-50 rounded-xl p-2 text-center border border-blue-100">
                        <span className="block text-[10px] text-gray-400 font-medium mb-0.5">$</span>
                        <span className="font-extrabold text-blue-700 text-xs">{formatDolar(subscription.dolar)}</span>
                      </div>
                      <div className="bg-gray-50 rounded-xl p-2 text-center border border-gray-100">
                        <span className="block text-[10px] text-gray-400 font-medium mb-0.5">التاريخ</span>
                        <span className="font-bold text-gray-700 text-xs">{formatDate(subscription.created_at)}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <select
                        value={subscription.type1}
                        onChange={(e) => handleUpdateSubscription(subscription.id, e.target.value as SubscriptionType)}
                        disabled={updatingId === subscription.id}
                        className={`flex-1 text-xs border rounded-xl px-2 py-2 font-bold outline-none cursor-pointer text-center ${getTypeBadge(subscription.type1)}`}
                      >
                        {SUBSCRIPTION_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                      </select>
                      <button
                        onClick={() => handleDeleteSubscription(subscription.id)}
                        disabled={updatingId === subscription.id}
                        className="flex-1 bg-red-500 hover:bg-red-600 text-white text-xs py-2 rounded-xl font-bold transition disabled:opacity-50 flex items-center justify-center gap-1"
                      >
        ...
        <div className="flex justify-end p-4 border-t border-gray-200 bg-gray-50 rounded-b-xl shrink-0">
          <button onClick={onClose} className="px-6 py-2.5 bg-gray-600 text-white font-bold rounded-xl hover:bg-gray-700 shadow-sm transition">إغلاق</button>
        </div>
      </div>

      {/* الدرج الجانبي للمحذوفات */}
      <div className={`fixed top-0 right-0 h-full w-full md:w-[450px] bg-white shadow-2xl z-[70] transform transition-transform duration-300 ease-in-out border-l border-gray-200 flex flex-col ${isDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50 shrink-0">
          <label className="block text-xs font-bold text-gray-500 mb-2 uppercase">تصفية حسب الفصل</label>
          <select
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50">
          {deletedSubscriptions.length === 0 ? (
            ...
            deletedSubscriptions.map((subscription, index) => (
              <div key={`del-${subscription.id}-${index}`} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="bg-gray-100 text-gray-600 text-xs font-bold px-2 py-1 rounded-lg border border-gray-200">{subscription.type1 || '-'}</span>
                </div>
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[65] transition-opacity" onClick={() => setIsDrawerOpen(false)}></div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 relative z-10">
            ...
            <div className="flex justify-end mt-6 gap-2">
              ...
              <button
                ...
                disabled={!quickNotification.title || !quickNotification.body}
