                ...
                <div className="mt-8 border-t border-gray-200 pt-6">
                  ...
                  <div className="flex flex-col gap-6">
                    {Array.from(new Set(AVAILABLE_PERMISSIONS.map(p => p.group))).map(group => (
                      <div key={group} className="border-b border-gray-200 pb-6 last:border-0">
                        <h4 className="text-[#2b3a82] text-right font-bold mb-4 text-lg flex items-center justify-end gap-2">{group}</h4>
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" dir="rtl">
                          {AVAILABLE_PERMISSIONS.filter(p => p.group === group).map(perm => (
                            <label key={perm.id} className="flex items-center justify-end gap-3 p-3 bg-white border border-gray-200 rounded-lg hover:border-blue-400 cursor-pointer">
                              <input 
                                type="checkbox"
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mt-8 border-t border-gray-200 pt-6">
                  ...
                  <div className="space-y-6 bg-gray-50/50 p-6 rounded-xl border border-gray-100">
                    {Array.from(new Set(AVAILABLE_PERMISSIONS.map(p => p.group))).map(group => (
                      <div key={group} className="border-b border-gray-200 pb-6 last:border-0 last:pb-0">
                        {/* عنوان المجموعة مع أيقونة */}
                        <h4 className="text-[#2b3a82] font-bold mb-4 flex items-center gap-2 text-lg">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            {group === 'الواجهة الرئيسية' && <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />}
                            {group !== 'الواجهة الرئيسية' && <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065zM15 12a3 3 0 11-6 0 3 3 0 016 0z" />}
                          </svg>
                          {group}
                        </h4>
                        
                        {/* خيارات الصلاحيات (التصميم الشبكي الجديد) */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                          {AVAILABLE_PERMISSIONS.filter(p => p.group === group).map(perm => (
                            <label 
                              key={perm.id} 
                              className="flex flex-row-reverse items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:border-blue-400 hover:shadow-sm cursor-pointer transition-all duration-200"
                            >
                              <span className="text-sm font-bold text-gray-700">{perm.name}</span>
                              <input 
                                type="checkbox"
                                className="rounded text-blue-600 focus:ring-blue-500 w-5 h-5 border-gray-300 cursor-pointer"
                                checked={(editingUser.permissions || []).includes(perm.id)}
                                onChange={(e) => {
                                  const currentPerms = editingUser.permissions || [];
                                  if (e.target.checked) {
