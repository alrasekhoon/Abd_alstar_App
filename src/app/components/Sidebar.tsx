...
import { 
  LayoutDashboard, BookOpen, Printer, Users, Settings, 
  ChevronDown, LogOut, Menu, X, Scale
} from 'lucide-react'
...
export default function Sidebar({ children }: { children?: React.ReactNode }) {
  ...
  // خاصية الحماية: حالة التحقق من الصلاحيات قبل عرض الصفحة
  ...
  useEffect(() => {
    ...
    // 1. التحقق من الدخول والصلاحيات للرابط الحالي
    const checkAuth = () => {
      if (!token && pathname !== '/login') {
        router.replace('/login') // طرد غير المسجلين
        return
      }

      if (token && role && pathname !== '/login' && pathname !== '/') {
        // جمع كل الروابط المسموحة لهذا المستخدم
        const allAllowedLinks = getMenuSections().flatMap(section => section.items.map(item => item.href));
        
        // إذا كان يحاول الدخول لرابط غير مسموح له، إعادته للرئيسية
        if (allAllowedLinks.length > 0 && !allAllowedLinks.includes(pathname)) {
           router.replace('/') 
           return
        }
      }
      setIsAuthChecking(false) // السماح بعرض المحتوى إذا اجتاز الفحص
    }

    checkAuth()

    // 2. استرجاع حالة فتح وإغلاق القوائم
    const savedState = localStorage.getItem('sidebarState')
    if (savedState) {
      setExpandedSections(JSON.parse(savedState))
    } else {
      setExpandedSections({ main: true }) // فتح القسم الأول افتراضياً
    }
  }, [pathname, router, getMenuSections])

  // فتح القسم تلقائياً بناءً على الصفحة الحالية
  useEffect(() => {
  ...
  // شاشة الانتظار الآمنة (أثناء فحص الصلاحيات)
  if (isAuthChecking && pathname !== '/login') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900">
        <Scale className="w-16 h-16 text-amber-500 animate-pulse drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]" />
      </div>
    )
  }

  // صفحة تسجيل الدخول (لا تعرض القائمة الجانبية)
  if (pathname === '/login') {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-right" dir="rtl">
              ...
              <div key={section.id} className="bg-slate-800/40 rounded-xl overflow-hidden border border-slate-700/50">
                ...
                >
                  <div className="flex items-center gap-3 font-bold">
                    <span className={`${hasActiveItem ? 'text-amber-500' : 'text-slate-500'}`}>{section.icon}</span>
                    {section.name}
                  </div>
                  <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${isExpanded ? 'rotate-180 text-amber-500' : 'text-slate-500'}`} />
                </button>
      ...
      {/* منطقة عرض المحتوى (للجوال والشاشات الكبيرة) */}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-100">
        ...
        {/* المحتوى الفعلي للصفحات يتم عرضه هنا */}
