...
export default function NewsManagement() {
  ...
  // قائمة الأنواع الثابتة
  const newsTypes = [
    { value: 'اخبار الجامعة', label: 'اخبار الجامعة' },
  ...
  const openAddForm = () => {
    setEditingItem({ 
      ...
      text: '', 
      note1: null, 
      type1: newsTypes[0].value, // تعيين النوع الافتراضي
      ashtrak: subscriptions.length > 0 ? subscriptions[0].id.toString() : null 
    });
  };
  ...
  const filteredItems = newsItems.filter(item => {
    return selectedTypeFilter === 'all' || item.type1 === selectedTypeFilter;
  });

  // Extract unique types for filter
  const uniqueTypes = Array.from(new Set(newsItems.map(item => item.type1)));
  ...
  return (
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-800">
                  {editingItem.id ? 'تعديل الخبر' : 'إضافة خبر جديد'}
                </h2>
                <button 
              ...
              {filteredItems.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-sm text-gray-500 line-clamp-1">{item.body.substring(0, 50)}...</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.type1}</td>
        ...
        {filteredItems.length === 0 && newsItems.length > 0 && (
          <div className="text-center py-12">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mx-auto text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <h3 className="mt-4 text-lg font-medium text-gray-900">لا توجد أخبار مطابقة للفلتر</h3>
            <p className="mt-1 text-sm text-gray-500">جرب تغيير خيارات التصفية أو أضف خبرًا جديدًا</p>
          </div>
        )}
        ...
        {newsItems.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">لا توجد أخبار</h3>
            <p className="mt-1 text-sm text-gray-500">ابدأ بإضافة خبر جديد بالنقر على الزر أعلاه</p>
          </div>
        )}
      </div>
    </div>
